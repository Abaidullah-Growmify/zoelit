...
export function buttonClasses({ variant = "primary", size = "md" } = {}) {
  return cn(
    "inline-flex items-center justify-center gap-2 rounded-lg font-bold tracking-[-0.01em] transition duration-200 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-500/15 disabled:pointer-events-none disabled:opacity-60",
  ...
  return (
    <div className={cn(centered ? "mx-auto max-w-3xl text-center" : "flex flex-col justify-between gap-5 md:flex-row md:items-end", className)}>
      <div className={cn(centered ? "mx-auto" : "")}>
        ...
        {description ? <p className={cn("mt-4 text-base font-medium leading-7 text-slate-600 dark:text-slate-300", centered ? "mx-auto max-w-2xl" : "max-w-2xl")}>{description}</p> : null}
      </div>
...
export function SectionHeader({ eyebrow, title, action, align = "left", className }) {
  ...
  return (
    <div className={cn(centered ? "mx-auto max-w-2xl text-center" : "flex items-end justify-between gap-4", className)}>
      <div>
        <h2 aria-label={eyebrow ? `${eyebrow}: ${title}` : undefined} className="text-balance font-heading text-3xl font-extrabold tracking-[-0.03em] text-slate-950 dark:text-white">{title}</h2>
      </div>
...
export function Input({ className, ...props }) {
  return <input className={cn("h-11 w-full rounded-lg border border-slate-300 bg-white px-4 text-sm font-semibold text-slate-900 transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-50 dark:placeholder:text-slate-500", className)} {...props} />;
}

export function Textarea({ className, ...props }) {
  return <textarea className={cn("min-h-28 w-full rounded-lg border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-900 transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-50 dark:placeholder:text-slate-500", className)} {...props} />;
}

export function Select({ className, style, ...props }) {
  return (
    <select
      className={cn("h-11 appearance-none rounded-lg border border-slate-300 bg-white bg-no-repeat py-0 pl-4 pr-11 text-sm font-bold text-slate-900 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-50", className)}
      style={{
...
export function Label({ className, ...props }) {
  return <label className={cn("text-sm font-bold text-slate-700 dark:text-slate-200", className)} {...props} />;
}
...
export function Badge({ children, tone = "slate" }) {
  ...
  return <span className={cn("inline-flex rounded-lg px-3 py-1 text-xs font-bold", tones[tone] || tones[children] || tones.slate)}>{children}</span>;
}
...
export function EmptyState({ title, description, action }) {
  return (
    <Card className="flex flex-col items-center justify-center py-14 text-center">
      <div className="mb-4 size-1.5 rounded-lg bg-blue-700 dark:bg-blue-300" />
      <h3 className="font-heading text-xl font-extrabold tracking-[-0.02em]">{title}</h3>
      <p className="mt-2 max-w-md text-sm font-medium leading-6 text-slate-600 dark:text-slate-300">{description}</p>
