          ...
          <>
            <div className="mt-8 space-y-5 overflow-y-auto pr-1">
              {items.map((item) => {
                ...
                return <div key={item.productId} className="flex gap-4"><Image src={product.image} alt={product.name} width={88} height={88} className="size-22 rounded-2xl object-cover" /><div className="flex-1"><h3 className="font-bold">{product.name}</h3><p className="text-sm text-slate-500 dark:text-slate-400">{money(product.price)}</p><div className="mt-3 flex items-center gap-2"><button className="size-8 rounded-full bg-slate-100 dark:bg-slate-800" onClick={() => updateQuantity(item.productId, item.quantity - 1)}>-</button><span>{item.quantity}</span><button className="size-8 rounded-full bg-slate-100 dark:bg-slate-800" onClick={() => updateQuantity(item.productId, item.quantity + 1)}>+</button><button className="ml-auto text-sm text-rose-600" onClick={() => removeItem(item.productId)}>Remove</button></div></div></div>;
              })}
            </div>
