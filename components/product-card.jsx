  ...
  return (
    <Card className="group flex h-full flex-col overflow-hidden rounded-lg border-slate-200 bg-white p-0 shadow-sm ring-1 ring-transparent transition duration-300 hover:-translate-y-1 hover:border-blue-200 hover:shadow-xl hover:shadow-blue-950/10 hover:ring-blue-100 dark:border-slate-800 dark:bg-slate-950 dark:hover:border-blue-500/40 dark:hover:ring-blue-500/10">
      <Link href={`/products/${product.id}`} className="block p-2 pb-0 focus-visible:outline-none">
        <div ref={imageRef} className="relative overflow-hidden rounded-lg bg-[radial-gradient(circle_at_top_left,#ffffff_0,#eff6ff_45%,#dbeafe_100%)] p-2 transition duration-300 group-hover:bg-[radial-gradient(circle_at_top_left,#ffffff_0,#dbeafe_45%,#bfdbfe_100%)] dark:bg-[radial-gradient(circle_at_top_left,#1e293b_0,#0f172a_52%,#172554_100%)]">
      ...
      <div className="flex flex-1 flex-col p-4">
        <div className="mb-2 flex items-center justify-between gap-3 text-xs font-bold text-slate-500 dark:text-slate-400">
        ...
        <div className="flex items-start justify-between gap-4">
          <Link href={`/products/${product.id}`} className="text-lg font-black leading-tight tracking-tight text-slate-950 transition hover:text-blue-600 focus-visible:outline-none focus-visible:text-blue-600 dark:text-white dark:hover:text-blue-300 dark:focus-visible:text-blue-300">{product.name}</Link>
          <span className="shrink-0 text-lg font-black tracking-tight text-blue-600">{money(product.price)}</span>
        </div>
        <p className="mt-2 line-clamp-2 min-h-10 text-sm leading-5 text-slate-500 dark:text-slate-400">{product.description}</p>
