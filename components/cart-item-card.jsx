...
export function CartItemCard({ item, updateQuantity, removeItem }) {
  ...
  const maxStock = Math.max(Number(product.stock ?? item.stock ?? 0) || 0, 1);
      ...
      <div className="flex gap-5">
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-between gap-4">
          <div className="min-w-0 pr-10">
            <h2 className="line-clamp-2 max-w-xl font-heading text-h2 font-semibold leading-tight tracking-tight text-slate-950 dark:text-white">{product.name}</h2>
            <p className="mt-1 line-clamp-1 max-w-xl text-body leading-5 text-slate-500 dark:text-slate-400">{product.description}</p>
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="inline-flex shrink-0 items-center gap-3 rounded-lg bg-slate-100 px-3 py-1.5 dark:bg-slate-800">
              <button className="grid size-7 place-items-center rounded-lg text-slate-600 transition hover:bg-white hover:text-blue-600 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-blue-300" onClick={() => updateQuantity(item.productId, quantity - 1)} aria-label={`Decrease ${product.name} quantity`}>
                <Minus className="size-4" />
              </button>
              <span className="min-w-5 text-center text-base font-semibold tabular-nums text-slate-950 dark:text-white">{quantity}</span>
              <button className="grid size-7 place-items-center rounded-lg text-slate-600 transition hover:bg-white hover:text-blue-600 dark:text-slate-300 dark:hover:bg-slate-700 dark:hover:text-blue-300" onClick={() => updateQuantity(item.productId, quantity + 1)} disabled={quantity >= maxStock} aria-label={`Increase ${product.name} quantity`}>
