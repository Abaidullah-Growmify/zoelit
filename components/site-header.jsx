  ...
  return (
    <>
      <header className="sticky top-0 z-40 border-b border-slate-200/70 bg-white/85 backdrop-blur-xl dark:border-slate-800 dark:bg-slate-950/85">
        ...
        {open ? <div className="container-page grid gap-3 pb-4 md:hidden">{nav.map((item) => <Link onClick={() => setOpen(false)} key={item.href} href={item.href} className="rounded-2xl bg-slate-100 px-4 py-3 text-sm font-bold dark:bg-slate-800">{item.label}</Link>)}<Link href={accountHref} className="rounded-2xl bg-blue-600 px-4 py-3 text-sm font-bold text-white">{authHydrated && user ? "Dashboard" : "Login"}</Link></div> : null}
      </header>
      <CartDrawer />
    </>
  );
