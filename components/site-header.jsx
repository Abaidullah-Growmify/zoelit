...
export function SiteHeader() {
  ...
  return (
    <div className="sticky top-0 z-50">
     ...
     {user ? <div className="account-strip"><div className="mx-auto flex max-w-[1280px] items-center justify-center gap-5 overflow-x-auto px-5 md:px-10 lg:px-16"><p className="hidden shrink-0 text-xs text-blue-200 lg:block">Hello, <b className="text-white">{user.name || "there"}</b></p><div className="flex min-w-max items-center justify-center gap-1"><AccountLink href="/dashboard" icon={LayoutDashboard} label="Dashboard" pathname={pathname} /><AccountLink href="/dashboard/orders" icon={Package} label="My Orders" pathname={pathname} /><AccountLink href="/dashboard/wishlist" icon={Heart} label="Wishlist" pathname={pathname} /><AccountLink href="/dashboard/profile" icon={Settings2} label="Account Settings" pathname={pathname} /><button type="button" onClick={handleLogout} className="account-strip-link"><LogOut className="size-3.5" />Sign Out</button></div></div></div> : null}
    </div>
  );
}
