    ...
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-white p-4 dark:bg-slate-950">
      ...
      <section className="relative z-10 w-full max-w-md rounded-xl border border-slate-200/70 bg-white/85 p-6 shadow-2xl shadow-slate-900/10 backdrop-blur-xl dark:border-slate-800/80 dark:bg-slate-900/80">
        <div className="flex flex-col items-center pb-2 text-center">
          ...
          <h1 className="text-2xl font-black tracking-tight text-blue-700 dark:text-blue-300">ZoeLit Login</h1>
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Sign in to your shopping account.</p>
        </div>
        <form onSubmit={form.handleSubmit(onSubmit)} className="mt-6 space-y-4" noValidate>
          <div className="space-y-2">
            <Label>Email</Label>
            <Input type="email" autoComplete="email" placeholder="you@example.com" className="h-10 rounded-lg bg-white text-[13px] shadow-sm dark:bg-slate-950" {...form.register("email")} />
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <Input type={showPassword ? "text" : "password"} autoComplete="current-password" placeholder="Enter password" className="h-10 rounded-lg bg-white pr-10 text-[13px] shadow-sm dark:bg-slate-950" {...form.register("password")} />
        ...
        <p className="mt-6 text-center text-[13px] text-slate-500 dark:text-slate-400">Need a ZoeLit account? <Link href="/register" className="font-bold text-blue-600 hover:text-blue-700 dark:text-blue-400">Create account</Link></p>
      </section>
    </div>
  );
}
