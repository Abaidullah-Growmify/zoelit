...
const contactItems = [
  { icon: Mail, title: "Email", text: "info@zoelit.com" },
  { icon: Phone, title: "UK Phone Number", text: "+44 749637 9004" },
  { icon: Phone, title: "UK Landline", text: "+44 161 791 5621" },
  { icon: Phone, title: "Pakistan Phone Number", text: "+92 3007404044" },
];
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-[.85fr_1.15fr]">
        <div className="grid gap-4">
          {contactItems.map((item) => (
            <Card key={item.title} className="flex items-start gap-4 shadow-none">
              <div className="grid size-12 shrink-0 place-items-center rounded-2xl bg-blue-50 text-blue-600 dark:bg-blue-500/10"><item.icon className="size-5" /></div>
              <div>
          ...
          <Card className="shadow-none">
            <div className="flex items-start gap-4">
              <div className="grid size-12 shrink-0 place-items-center rounded-2xl bg-blue-50 text-blue-600 dark:bg-blue-500/10"><MessageCircle className="size-5" /></div>
              <div>
        ...
        <Card>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="mt-6 grid gap-5 md:grid-cols-2">
            <Field label="Full name" name="name" form={form} />
            <Field label="Email" name="email" type="email" form={form} />
            <Field label="Mobile no" name="phone" type="tel" form={form} />
            <Field label="Company" name="company" form={form} />
            <div className="md:col-span-2">
              <Label>Message</Label>
              <textarea {...form.register("message")} rows={6} className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-50" />
              <ErrorText>{form.formState.errors.message?.message}</ErrorText>
            </div>
            <div className="md:col-span-2 rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-500 dark:bg-slate-900 dark:text-slate-400">
              ...
              <p className="mt-3">
                By submitting this online enquiry you consent to the sharing of your information with and to be contacted by Zoel IT LTD for the purpose of responding to your enquiry. For further details on how we collect, use and disclose personal information you should refer to our privacy policy.
              </p>
            </div>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          {locations.map((location) => (
            <Card key={location.title} className="shadow-none">
              <div className="flex items-start gap-4">
                <div className="grid size-12 shrink-0 place-items-center rounded-2xl bg-blue-50 text-blue-600 dark:bg-blue-500/10"><MapPin className="size-5" /></div>
                <div>
...
function Field({ label, name, form, type = "text", className }) {
  return (
