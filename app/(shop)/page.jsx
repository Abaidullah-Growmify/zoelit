      ...
      <section className="relative overflow-hidden bg-[radial-gradient(circle_at_top_left,#dbeafe,transparent_35%),linear-gradient(135deg,#ffffff,#f8fafc)] py-20 dark:bg-[radial-gradient(circle_at_top_left,#1e3a8a,transparent_32%),linear-gradient(135deg,#020617,#0f172a)]">
        <div className="container-page grid items-center gap-12 lg:grid-cols-[1.05fr_.95fr]">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {products.slice(0, 4).map((product, index) => <Card key={product.id} className={`p-4 ${index % 2 ? "sm:translate-y-8" : ""}`}><Image src={product.image} alt={product.name} width={500} height={500} className="aspect-square w-full rounded-md object-cover" /><p className="mt-3 font-bold">{product.name}</p></Card>)}
          </div>
        </div>
      </section>
      {productSections.map((section, index) => (
        <section key={section.eyebrow} className={`container-page ${index === 0 ? "py-16" : "pb-16"}`}>
          <div className="mb-8 flex items-end justify-between gap-4"><div><p className="font-bold text-blue-600">{section.eyebrow}</p><h2 className="text-3xl font-black">{section.title}</h2></div><Link href="/products" className="font-bold text-blue-600">View all</Link></div>
