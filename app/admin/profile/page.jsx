        ...
        <div className="mt-4 space-y-3">
          {accountSummary.map((item) => (
            <div key={item.label} className="flex items-center justify-between gap-3 text-sm">
              ...
              <strong className="text-right font-bold text-slate-950 dark:text-white">{item.value}</strong>
            </div>
          ))}
        </div>
      </div>
...
function AdminSecurityCard() {
  ...
  return (
    <Card className="transition duration-150 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-slate-200/70 dark:hover:shadow-black/20">
      ...
      <p className="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-400">Update the mock admin password controls. Real authentication can be connected later.</p>
      <form onSubmit={form.handleSubmit(onSubmit)} className="mt-5 grid gap-4 md:grid-cols-2">
