...
export const getAdminProduct = (id, token) =>
  request(`/api/admin/products/${encodeURIComponent(id)}`, { token });

export const updateAdminProduct = (id, payload, token) =>
  request(`/api/admin/products/${encodeURIComponent(id)}`, { method: "PATCH", body: payload, token });
...
export const getAdminCustomer = (id, token) => request(`/api/admin/customers/${encodeURIComponent(id)}`, { token });
...
export const getAdminOrder = (id, token) => request(`/api/admin/orders/${encodeURIComponent(id)}`, { token });

export const updateAdminOrderStatus = (id, payload, token) =>
  request(`/api/admin/orders/${encodeURIComponent(id)}/status`, { method: "PATCH", body: payload, token });

export const cancelAdminOrder = (id, token) =>
  request(`/api/admin/orders/${encodeURIComponent(id)}/cancel`, { method: "POST", body: {}, token });

export const getAdminDashboardSummary = (token) => request("/api/admin/dashboard/summary", { token });

export const validateCheckoutPrices = (products, token) =>
  request("/api/checkout/validate", { method: "POST", body: { products }, token });

export const createPaymentIntent = (products, token) =>
  request("/api/checkout/create-payment-intent", { method: "POST", body: { products }, token });

export const confirmPayment = (payload, token) =>
  request("/api/checkout/confirm", { method: "POST", body: payload, token });
